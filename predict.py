...
class Predict:
    ...
    def predict(self, player: str):
        ...
        self.data["FORECAST_DATE"] = pd.to_datetime(self.data["FORECAST_DATE"], format="%Y-%m-%d")
        ...
        forecaster.fit(y=self.data.loc[: max(self.data.index) - relativedelta(months=self.steps-1)])
    ...
    def predict_all(self):
        ...
        return pd.DataFrame(self.results, columns=['PLAYER', 'ACTUAL SEASON TOTAL', 'PREDICTED', 'SEASON MAPE',
                                             'GAME MAPE', 'GAMES PREDICTED', 'LAGS', 'REGRESSOR'])
